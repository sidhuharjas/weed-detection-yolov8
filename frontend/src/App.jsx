...
// Corrected import paths pointing to your components folder
...
export default function App() {
  ...
  const runInference = useCallback(async () => {
    ...
    setRunning(true);
    setError('');
    setProgress(30); // Mock initial progress
    setTab('image');
    ...
    try {
      ...
      // MOCK INFERENCE FOR DEMO PURPOSES IF BACKEND IS OFFLINE
      if (!backendOk) {
         setTimeout(() => {
            setProgress(100);
            const mockDets = [
  ...
  return (
    <div className="app">
      <Sidebar 
        conf={conf} setConf={setConf} iou={iou} setIou={setIou}
        sliceSize={sliceSize} setSliceSize={setSliceSize} overlap={overlap} setOverlap={setOverlap}
        useSahi={useSahi} setUseSahi={setUseSahi} modelArch={modelArch} setModelArch={setModelArch}
        showBoxes={showBoxes} setShowBoxes={setShowBoxes} showConf={showConf} setShowConf={setShowConf}
        weedsOnly={weedsOnly} setWeedsOnly={setWeedsOnly} showHeat={showHeat} setShowHeat={setShowHeat}
        heatOpacity={heatOpacity} setHeatOpacity={setHeatOpacity}
        file={file} onFile={handleFile} running={running} onRun={runInference}
      ...
      <div className="main">
        ...
        {tab === 'image' ? (
          <Canvas 
            imgEl={imgEl} detections={detections} conf={conf} 
            showBoxes={showBoxes} showConf={showConf} weedsOnly={weedsOnly}
            loading={running} progress={progress} progressText="Running Model..." 
          />
        ) : (
      ...
      <Results 
        detections={detections} conf={conf} inferTime={inferTime} 
        history={history} onLoadHistory={(h) => { setDetections(h.dets); setAnnotatedSrc(h.annotated); setTab('image'); }} 
      />
    </div>
  );
}